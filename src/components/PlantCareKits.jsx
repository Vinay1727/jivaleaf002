...
const PlantCareKits = ({ addToCart }) => {
  ...
  const fetchCareKits = async () => {
    try {
      setLoading(true);
      const response = await fetch(`${API_BASE}/api/plants/care-kits`);
      ...
      console.error("Error fetching care kits:", err);
      setError("Error loading care kits");
    } finally {
      setLoading(false);
    }
  };
    ...
    <section className="min-h-screen px-0 py-8 md:py-16 bg-gradient-to-b from-transparent to-green-950/10">
      <div className="max-w-full mx-auto px-4 md:px-8">
        <div className="mb-12 text-center">
          <h2 className="text-2xl md:text-5xl font-bold mb-2 md:mb-4">🧴 Plant Care Kits Collection</h2>
          <p className="text-sm md:text-xl text-gray-300 mb-1 md:mb-2">Everything you need to care for your plants</p>
          <p className="text-gray-400 text-xs md:text-base">Professional tools for plant enthusiasts</p>
        </div>

        {loading && (
          <div className="text-center py-12">
            <p className="text-xl text-gray-300">Loading care kits...</p>
          </div>
        )}
        ...
        {!loading && !error && topPlants.length > 0 && (
          <>
            ...
            <div className="mb-8 md:mb-16 p-4 md:p-8 bg-gradient-to-r from-green-900/30 to-emerald-900/30 border border-green-700 rounded-xl md:rounded-2xl">
              <h3 className="text-lg md:text-2xl font-bold mb-4 md:mb-6 text-center">🌟 Top 10 Premium Care Kits</h3>
                  ...
                  <div className="lg:col-span-2 space-y-4 transition-all duration-500">
                    ...
                    {currentPlant.description && (
                      <div className="bg-black/40 rounded-xl p-4 max-h-48 overflow-y-auto transition-all duration-500">
                        <h3 className="text-base md:text-lg font-bold mb-2 transition-all duration-500">About This Product</h3>
                        <div
                    ...
                    <div className="grid grid-cols-2 gap-2 pt-2">
                      ...
                      <div className="bg-gradient-to-br from-green-600/30 to-emerald-700/30 rounded-lg p-3 border border-green-600/50 text-center">
                        <p className="text-xl mb-1">🌿</p>
                        <p className="text-white font-bold text-xs">Expert Tips</p>
                      </div>
                    </div>
                  </div>
                )}
        ...
        {!loading && !error && allPlants.length === 0 && (
          <div className="text-center py-12 text-gray-400">
            <p className="text-lg">No care kits available at the moment</p>
          </div>
        )}

        {/* All Plants Grid */}
        {!loading && !error && allPlants.length > 0 && (
          <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 md:gap-6 mt-12">
            {allPlants.map((p) => (
              <div
                key={p._id}
